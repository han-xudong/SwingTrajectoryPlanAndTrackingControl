...
class SimpleRobot(object):
  def __init__(self, pybullet_client, robot_uid, simulation_time_step):
    ...
    self._motor_model = A1MotorModel(kp=self._kp,
                                     kd=self._kd,
  ...
  def _SettleDownForReset(self, reset_time):
    ...
    if reset_time <= 0:
      return
    for _ in range(500):
      self._StepInternal(INIT_MOTOR_ANGLES,
                         motor_control_mode=MOTOR_CONTROL_POSITION)
  ...
  def GetMotorPositionGains(self):
     return np.array([ABDUCTION_P_GAIN,
                      HIP_P_GAIN,
                      KNEE_P_GAIN,
                      ABDUCTION_P_GAIN,
                      HIP_P_GAIN,
  ...
  def GetMotorVelocityGains(self):
    return np.array([ABDUCTION_D_GAIN,
                     HIP_D_GAIN,
                     KNEE_D_GAIN,
                     ABDUCTION_D_GAIN,
                     HIP_D_GAIN,
  ...
  def compute_jacobian(self, robot, link_id):
    ...
    jv, _ = self.pybullet_client.calculateJacobian(robot.quadruped,
                                                   link_id,
                                                   (0, 0, 0),
                                                   all_joint_angles,
                                                   zero_vec,
  ...
  def ComputeJacobian(self, leg_id):
    ...
    return self.compute_jacobian(robot=self,
  ...
  def world_frame_from_local_frame(self,
                                  local_frame_position,
                                  base_translation=(0, 0, 0),
                                  base_rotation=(0, 0, 0, 1)):
    """Converts a vector from the local frame to the world frame."""
    base_position, base_orientation = self.pybullet_client.getBasePositionAndOrientation(self.quadruped)
    base_position, base_orientation = self.pybullet_client.multiplyTransforms(base_position,
                                                                              base_orientation,
                                                                              base_translation,
                                                                              base_rotation)
    world_frame_pos, _ = self.pybullet_client.multiplyTransforms(base_position,
                                                                 base_orientation,
                                                                 local_frame_position,
  ...
  def joint_angles_from_link_position(self,
    ...
    if not position_in_world_frame:
      ...
      base_position, base_orientation = self.pybullet_client.getBasePositionAndOrientation(self.quadruped)
      base_position, base_orientation = robot.pybullet_client.multiplyTransforms(base_position,
                                                                                 base_orientation,
                                                                                 base_translation,
                                                                                 base_rotation)

      # Projects to world space.
      world_link_pos, _ = robot.pybullet_client.multiplyTransforms(base_position,
                                                                   base_orientation,
                                                                   link_position,
                                                                   _IDENTITY_ORIENTATION)
    else:
    ...
    ik_solver = 0
    all_joint_angles = robot.pybullet_client.calculateInverseKinematics(robot.quadruped,
                                                                        link_id,
                                                                        world_link_pos,
                                                                        solver=ik_solver)
  ...
  def ComputeMotorAnglesFromFootLocalPosition(self,
                                              leg_id,
                                              foot_local_position):
  ...
  def _EndEffectorIK(self, leg_id, position, position_in_world_frame):
    ...
    motors_per_leg = self.num_motors // self.num_legs
    joint_position_idxs = [i for i in range(leg_id * motors_per_leg, leg_id * motors_per_leg + motors_per_leg)]
    joint_angles = self.joint_angles_from_link_position(robot=self,
                                                        link_position=position,
                                                        link_id=toe_id,
                                                        joint_ids=joint_position_idxs,
                                                        position_in_world_frame=position_in_world_frame)
    # Joint offset is necessary for A1.
    joint_angles = np.multiply(np.asarray(joint_angles) - np.asarray(self._motor_offset)[joint_position_idxs],
  ...
  def TransformAngularVelocityToLocalFrame(self, angular_velocity, orientation):
    ...
    relative_velocity, _ = self.pybullet_client.multiplyTransforms([0, 0, 0],
  ...
  def link_position_in_world_frame( self,   link_id ):
  ...
  def GetFootPositionsInWorldFrame(self):
    ...
    foot_positions = []
    for foot_id in self.GetFootLinkIDs():
      foot_positions.append(self.link_position_in_world_frame(link_id=foot_id))
